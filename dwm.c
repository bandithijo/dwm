...
col(Monitor *m)
{
	...
	for (i = x = y = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++)
		if (i < m->nmaster) {
			...
			resize(c, x + m->wx, m->wy, w - (2 * c->bw) + (n > 1 ? gappx : 0), m->wh - (2 * c->bw), 0);
			x += WIDTH(c);
		} else {
			h = (m->wh - y) / (n - i);
...
drawbar(Monitor *m)
{
	...
	if (m == selmon || 1) { /* status is only drawn on selected monitor */
		...
		drw_text(drw, m->ww - tw - 2 * sp, 0, tw, bh + 2, 0, stext, 0);
	}
	...
	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			drw_setscheme(drw, scheme[m == selmon ? SchemeNorm : SchemeNorm]); /* default: selmon ? SchemeSel : SchemeNorm */
